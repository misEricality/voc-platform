...
| 2026-09-10 | **静态快照自动发布落地（方案 A）**：`ops/register_local_collect_task.ps1` 新增第 4 个任务 **`VOC-Local-Publish-Snapshot`（04:30）** → 调 `ops/publish_static_snapshot.ps1` 导出 + 发布 EdgeOne Pages；新增 `-PublishAt` / `-SnapshotProject` 参数，`-Uninstall` 同步覆盖。**修复 `publish_static_snapshot.ps1` 冷启动 bug**（dist 检查早于导出 → 产物被清理后无法重新导出）+ 显式传 `--out` 使自定义 `-Dist` 生效。**修正文档/代码不一致**：早期文档称 `daily_incremental_collect.py` 有 `--publish-snapshot`，实际从未落地（该脚本无任何 snapshot/publish 分支）——改为独立任务方案后已对齐。另在 `smart_sync_release` / `register_sync_tasks` 条目加「勿再注册 sync」告警 | 工程师确认「做方案A + 顺手修掉」；公网静态站此前只能手动发布，且远端内容停在 9/7 |
