...
- 页面（2026-09-05 现役 5 页，前 3 页已按用户线框图重构）：B站视频看板（`#/bilibili`，封面+快照信息卡+性别/情感环形+L1 正负拆分联动筛选+可折叠原声列表+30s 弹幕时间轴+LLM 高光时刻）；「Steam游戏看板」一级导航 hover 下拉两个子模块 —— 游戏对比（`#/compare`）/ 单游戏（`#/dashboard`，标题「Steam游戏看板 - 单游戏/游戏对比」，无副标题；compare 指标表游戏名可跳 `#/dashboard?target=&range=30d` 带筛选）；「系统管理」下拉 —— 采集任务（`#/admin`，登录后）/ 数据管理（`#/data`，公开无需登录，`#/trends` 兼容跳转）
