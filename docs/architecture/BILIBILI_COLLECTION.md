...
> **前置实测**：`scripts/dev/probe_bilibili.py` / `probe_bili_wbi.py`（2026-08-11 沙箱实测，接口可行性已验证）
...
VoC 平台已跑通 Steam 评论全链路（2067 条：采集→打标→观点→向量→仪表盘）。B 站作为第二数据源，价值不在"第二个评论源"，而在三类增量资产：
