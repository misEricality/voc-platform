...
| 指标 | 数值 | 业务解读 |
|---|---|---|
| 总评论数 | **9308 条** | Steam 8302（10 款游戏，补采至 08-16）+ B站 1006（BV1UpwaeNESx） |
| 情感分析覆盖 | **9242/9308（99.3%）** | ✅ 余 66 条待分析（B站 18 + Steam 48） |
| 观点级标注 | 15298 条 | `comment_opinions` 表（程序匹配到的观点短语） |
| 语义向量覆盖 | **9309 条（全量，单模型）** | `comment_embeddings` 表（bge-small-zh-v1.5，已全量回填） |
| 已支持游戏/视频 | **10 款游戏 + 1 条视频** | Steam 10 款 + B站 BV1UpwaeNESx |
| 数据截至 | 2026-08-16 | Steam 最新评论 08-16 23:58；B站 08-12（B站无时间窗采集） |
...
| 远端 GitHub 状态 | 本地领先 `488ff02` | 大量清理/词典/P3 改动已暂存未提交（待 commit） |
...
### ⭐ 当前主线（P6/P9 阶段 0 持久化 + 工程护栏）

1. **P6 / P9 阶段 0 时序持久化**：✅ 阶段 1（兜底治理）与 P3（多目标对比）已收口（2026-08-19）；**下一步是定时采集落库持久化**，解锁时间序列趋势（P8）与监控形态——当前每次运行仍生成全新库，不累积
2. **CI 补 pytest + analyzer_version 溯源**：两个 🟡 工程护栏，成本低、防回归价值高
3. **CS2（appid 730）补采复查**：08-16 补采时 CS2 0 新增（仍 459 条 / 最新 08-04），需单独查一次采集链路
