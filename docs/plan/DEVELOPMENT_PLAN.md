...
| 指标 | 数值 | 业务解读 |
|---|---|---|
| 总评论数 | 772 条 | 6 款游戏，已足够做横向对比 |
| 情感分析覆盖 | **0/772（0%）** | ⚠️ **P0 阻塞**：schema 升级后旧分析失效，需重跑 |
| 已支持游戏 | 6 款 | 黑神话悟空 / 巫师3 / 文明6 / 底特律 / 光与影 33号远征队 / 星际拓荒 / CS2 |
| 正向 / 负向 / 中性 | 待分析 | 重跑后才能看 |
| 主题 TOP1 | 待分析 | 旧版"游戏性" 72% 占比说明分类过粗 |
...
### 🔴 P0 · 修复情感分析覆盖（必须先做，否则 M6 之后的仪表盘都没数据）
...
| 阻塞项 | 解决方式 | 阻塞了谁 |
|---|---|---|
| 🔴 **voc.db 0 条已分析** | 跑一次 `pipeline` 重新分析 772 条（成本约 ¥2-5） | 全部 P1+ 仪表盘迭代 |
...
| `scripts/ops/refresh_likes.py` 未实现 | 实现回采脚本（0.5 天工作量） | P4 文档化 + 7 天后回采机制 |
