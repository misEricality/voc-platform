...
│
├── src/                            # 【核心代码】正式模块（7 子目录 + 1 顶层）
│   ├── pipeline.py                     主流程编排（CLI 入口 python -m src.pipeline）
│   ├── api/                            Web 数据服务层（FastAPI：公开只读端点 + admin CRUD + 鉴权）
...
│   ├── agent/                          原声分析 Agent（chat / tools / skills / sessions / md_corpus）
│   ├── collectors/                     采集器（Steam Web API + B站公开接口）
│   ├── storage/                        存储层（SQLAlchemy 2.x + SQLite 单库）
...
│   ├── agent/                          Agent tool schema + skill 模板（tools.yaml + skills/）
│   ├── wordlists/                      停用词表（评论词云）
...
- [x] **v0.8** - 原声分析 Agent（悬浮球两段式抽屉 + 4 tool function calling + skill 系统 + 引用当前查询 + 30 天保留 + Markdown 导出）+ Web 看板 + 本地直采/哨兵闭环（2026-09-10）
- [ ] **v1.0** - P9 阶段 2/3 落地 + 公网部署（VPS）+ 完整文档 + 技术博客 + 求职作品集发布
...
- **创作周期**: 2026-07 ~ 至今（v0.8 Agent 上线 → v1.0 作品化进行中）
