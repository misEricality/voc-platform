...
MEME_STRONG_KEYWORDS = [
    ...
    "沙雕", "表情包", "段子", "发癫", "逆天",
    # 黑神话/影之刃"村里第一个大学生"系列梗（第一个大学生/研究生/武术生）
    "村里", "第一个大学生", "第二个大学生", "第一个研究生",
    # 2026-08-18 扩充：吃鸡梗 / B站弹幕表情（避免落"总体体验评价"兜底）
    "大吉大利", "星星眼",
]
...
RECOMMEND_WORDS = ["推荐", "值不值得", "值得买", "值不值", "必买", "千万别买", "非常值得", "出必买", "值得一玩", "年度最佳"]

# 消歧：专有名词中的“卡”不是“卡顿”。
# 匹配前先把这些子串替换为占位符，避免子串包含匹配误命中。
# 注意：卡拉/康纳/马库斯（《底特律》角色）**不放进来**——它们是主线剧情词典里的
# 双字专有名词关键词（权重 2），已天然压过单字“卡”（权重 1），能正确命中主线剧情；
# 若在此屏蔽会连带失效、导致误落兜底。这里只屏蔽“卡总/卡大厅”这类词典未收录、
# 但会被单字“卡”误命中的专名/表述。
DISAMBIGUATION_SUBSTRINGS = ("卡总", "卡大厅")
