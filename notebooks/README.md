...
**已到启用门槛（2026-08-15）**：评论数据已达 **3073 条**（Steam 2067 + B站 1006，已分析 3034），观点级标注 5212 条，语义向量 1021 条（覆盖约 1/3）。

适合用 Jupyter 的场景（按优先级）：
- 「其他/整体评价」兜底桶占比已升至 67.0%（topic 口径，2026-08-15 实测，趋势恶化）→ 匹配层语义匹配升级 + 聚类治理（当前数据最大痛点）
